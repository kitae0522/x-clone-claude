...
## Phase 16: 프로필 이미지 업로드 버그 수정 (#55) (진행중)
- [x] 스펙 문서 작성 (docs/specs/profile-image-upload-fix-spec.md)
- [x] Bug 1 수정: DTO validator `url` 태그 제거 (상대 경로 허용)
- [x] Bug 2 수정: Vite `/uploads` 프록시 추가
- [x] Bug 3 수정: 이미지 URL 빈 문자열 시 기존 값 보존
- [x] 빌드 & 린트 검증 통과
- [ ] 테스트 작성
- [ ] 코드 리뷰
- [ ] PR 생성 및 이슈 연결
...
## 최근 변경 로그
- 2026-03-07: 이슈 #55 프로필 이미지 업로드 버그 수정 - validator url 태그, Vite 프록시, 이미지 URL 보존
